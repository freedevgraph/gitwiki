...
def test_dark_mode_button_in_base_template(client):
    ...
    assert b'value="amoled"' in response.data
...
def test_dark_mode_css_present(client):
    ...
    assert b'[data-theme="amoled"]' in response.data
    assert b'--bg: #000000;' in response.data
    assert b'--surface: #0a0a0a;' in response.data
